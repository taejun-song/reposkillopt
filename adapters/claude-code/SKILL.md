...
<!--
This adapter is a thin wrapper around the canonical skill at
skills/repo-skillopt/SKILL.md (canonical_version 0.1.0). It preserves
every required section, principle, workflow stage, and output-discipline
rule. Per-adapter notes (if any) live at the bottom under "Notes for
this adapter" and are non-normative.
-->
...
**Refactoring opportunities (advisory).** In *Refactoring opportunities*, report clusters of **structurally-near-duplicate** functions — same shape, different names/literals — that could be abstracted (a shared helper, base method, or decorator). Detect them deterministically: normalize each function body to its shape (identifiers and literals replaced, keywords and the type after `except`/`raise` kept) and group identical shapes; locate candidates with `rg`/`grep`. Report only above a **low-noise threshold** (≥3 instances, ≥8 tokens), cite every instance `file:line`, and suggest the abstraction. This section is **advisory and `**[inference]**` only** — never `**[fact]**`, never scored or gated; it must not alter required sections, the rubric, or the reward. Write "None known." when nothing qualifies.
...
7. **Apply accepted proposals to the canonical skill.** When `status: accepted` (with a passing gate referenced), the proposal's diff is applied to the canonical `SKILL.md`. The canonical version is bumped per Keep-A-Changelog + semver: major if the diff breaks the adapter-equivalence checklist, minor if additive, patch if clarifying. A row is added to the changelog.
...
- **Install location**: copy this file to `<target-repo>/.claude/skills/repo-skillopt/SKILL.md`. The harness routes by the `description:` field; no further activation is needed.
- **Verification**: `grep '^canonical_version:' <target-repo>/.claude/skills/repo-skillopt/SKILL.md` must equal the canonical `version:` in this project's `skills/repo-skillopt/SKILL.md`.
- **Known cross-agent differences**: none recorded yet. Add observations to `adapters/claude-code/README.md` under "Known cross-agent differences" as they surface in real use.
