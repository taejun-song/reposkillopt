...
6. **Apply accepted proposals to the canonical skill.** When `status: accepted`, the proposal's diff is applied to the canonical `SKILL.md`. The canonical version is bumped per Keep-A-Changelog + semver: major if the diff breaks the adapter-equivalence checklist, minor if additive, patch if clarifying. A row is added to the changelog.
