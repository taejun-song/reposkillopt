...
- **Install location**: copy this file to `<target-repo>/AGENTS.md`. Codex picks it up from the repository root.
- **Metadata**: the HTML-comment block at the top of this file carries `canonical_version` and `adapter`. It is ignored by Markdown renderers and by Codex prose parsing, but remains greppable by the adapter-equivalence check.
...
- **Known cross-agent differences**: none recorded yet. Add observations to `adapters/codex/README.md` under "Known cross-agent differences" as they surface in real use.
