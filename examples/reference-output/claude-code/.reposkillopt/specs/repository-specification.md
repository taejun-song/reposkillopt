...
**[human]** The deliberate enumeration of the internal, underscore-prefixed modules above (`_compat.py`, `_winconsole.py`, `_termui_impl.py`, `_textwrap.py`) — each with an assigned role rather than a public-surface-only listing — was reviewer-endorsed as the strongest part of this spec. It is recorded here as a deliberate practice, not an accident: enumerating secondary structures (internal modules, and analogously non-pytest test trees) is what a maintenance reader relies on. (Source: `FB-2026-06-01-004`.)
...
- `FB-2026-06-01-004` — quality-rating Feedback Item; added internal-module-enumeration `**[human]**` note in *Architectural layers*.
...
- **revision 2 → 3** (2026-06-01): Applied one Feedback Item — `FB-2026-06-01-004` (quality-rating; added a `**[human]**` note to *Architectural layers* recording the internal-module enumeration as a deliberate, reviewer-endorsed practice). All prior `**[human]**` claims from revision 2 (`FB-2026-05-31-002` in *Testing strategy*, `FB-2026-05-31-003` in *Domain model*, `FB-2026-05-31-001`-derived content in *Major entrypoints*) are preserved unchanged.
